...
POD 文档在 Perl 中随处可见，最常用的，比如 perldoc 查看 Perl 
...
=item C<L<name/"sec">> 或 C<L<name/sec>>

=item C<L</"sec">> 或 C<L<name/sec>>

=back

=item C<E<escape>> -- 字符转义

类似 HTML/XML 的实体

=item C<F<filename>> -- 文件名

=item C<S<text>> -- 禁目换行

如 C<S<$x ? $y : $z>>

=item C<X<topic name>> -- 索引名
...
代码中使用 POD 时最重要的是 =cut 这个标记，不管是标题还是自定义的文档块，都要有 =cut 结束该文档块。代码中可以使用 =headx 这种标题标记，记住结束标记 =cut，同样，自定义的文档块 ={自定义标记} 结束时也不能忘了加 =cut。如下，展示了一个文档中使用 POD 的实例：
...
=item * Pod::Simple - POD 解析框架

=item * Test::Pod - 检测 POD 文件中的语法错误

=item * Pod::Weaver weave together a Pod document from an outline.

=item * Pod::Perldoc - perldoc 命令使用的 POD 模块，用于查找 POD 文档中所需的
Perl 文档。

=item * Pod::Html - 把 POD 文档转换为 HTML 文件，Perl 核心模块，提供 pod2html
命令。
...
=item * pod2fm - 把 POD 文档转换为 FrameMaker 格式

=item * pod2latex - 转换为 LaTeX 格式

=item * pod2man - 转换成 UNIX man 手册格式

=item * pod2text - 转换成文本文档
