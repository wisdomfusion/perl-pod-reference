...
可以看出来 C<< C<< code here >> >> 这种写法比较容读，也是我们推荐的代码文件格式，但是要注意的是，C<E<lt>E<gt>> 后和 C<E<gt>E<gt>> 前都要有空格，可以写多个，但至少有一个。同时，C<E<lt>> 的个数可以是2个、3个或是更多个，但是开有几个，关也要有对应的个数个 C<E<gt>>。如下面这两种写法也是合法的：
